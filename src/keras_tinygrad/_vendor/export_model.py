...
def iter_kernel_calls(linear: UOp):
    ...
    return (
        u
        for u in linear.toposort(gate=lambda x: x.op not in _KERNEL_ASTS)
        if u.op is Ops.CALL and u.src[0].op in _KERNEL_ASTS
    )


def compile_net(
    linear: UOp, output_bufs: List[Buffer]
) -> Tuple[Dict[str, str], List, Dict[str, Tuple[int, DType, int]], Dict[str, Buffer]]:
...
def export_model_clang(
    ...
    dtype_map = {
        dtypes.int: "int",
        dtypes.float: "float",
        dtypes.uchar: "unsigned char",
        dtypes.char: "signed char",
        dtypes.half: "__fp16",
    ...
    forward_args = ",".join(
        f"{dtype}{'*' if name not in symbolic_vars.values() else ''} {name}"
        for name, dtype, _ in (outputs + inputs if wasm else inputs + outputs)
    )
    ...
    if not wasm:
        ...
        cprog += [
            f"{dtype_map[dtype]} {name}[{len}];"
            if name not in bufs_to_save
            else f"{dtype_map[dtype]} *{name} = ({dtype_map[dtype]} *){name}_data;"
            for name, (len, dtype, _key) in bufs.items()
            if name not in input_names + output_names
        ]
        cprog += (
            [f"void net({forward_args}) {{"]
            + [f"{name}({', '.join(args)});" for (name, args, _global_size, _local_size) in statements]
            + ["}"]
        )
        return "\n".join(headers + cprog)
    else:
        if bufs_to_save:
            ...
            bufs_to_save = {
                k: v for k, v in bufs.items() if v[2] in weight_names
            }  # causes random seeds to be set as zeroes, not exported as a model weight
            buf_to_name = OrderedDict(
                (buf_name, {"name": weight_names[data[2]], "idx": i})
                for i, (buf_name, data) in enumerate(bufs_to_save.items())
            )
            cprog.append(f"void* bufs[{len(buf_to_name)}];")
            cprog.append("""void set_buf(size_t index, void* ptr) {\n  bufs[index] = ptr;\n}""")
        ...
        get_weight_ptr = lambda x: (
            f"({dtype_map[bufs_to_save[x][1]]} *)bufs[{buf_to_name[x]['idx']}]" if x in bufs_to_save else x
        )
        cprog += [
            f"  {name}({', '.join(map(get_weight_ptr, args))});"
...
def export_model_webgpu(
    ...
    create_bind_group_layouts = ",".join(
        [
            "device.createBindGroupLayout({{entries: [{{binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: {{ type: 'uniform' }}}}, {}]}})".format(
                ",".join(
                    [
                        f"{{binding: {argIdx + 1}, visibility: GPUShaderStage.COMPUTE, buffer: {{ type: '{buf_type(argName)}' }} }}"
                        for argIdx, argName in enumerate(args)
                    ]
                )
            )
    ...
    kernel_calls = "\n        ".join(
        [
            f"addComputePass(device, commandEncoder, pipelines[{i}], layouts[{i}], infinityBuf, [{', '.join(args)}], [{', '.join(str(x) for x in global_size)}]);"
    ...
    map_to_external_weight = lambda _key: (
        f"state_dict['{weight_names[_key]}']"
        if stream_weights
        else f"getTensorBuffer(safetensor, metadata['{weight_names[_key]}'])"
    )
    _bufs = "\n    ".join(
        [
            f"const {name} = "
            + (
                f"{buf_type(_key)}(device, {size});"
                if _key not in weight_names
                else f"createWeightBuf(device, {size}, {map_to_external_weight(_key)})"
            )
            + ";"
...
def export_model(model, target: str, *inputs, model_name: Optional[str] = "model", stream_weights=False):
    ...
    weight_names = {
        (id(b), b.offset, b.size, b.dtype): name for name, x in state.items() if (b := x.uop.base.realized) is not None
    }
        ...
        prg = json.dumps(
            {
                ...
                "inputs": [{"size": bufs[name][0], "dtype": bufs[name][1].name} for name in input_names],
                "outputs": [{"size": bufs[name][0], "dtype": bufs[name][1].name} for name in output_names],
                "functions": functions,
                "statements": [
                    {"kernel": kernel, "args": args, "global_size": global_size, "local_size": local_size}
                    for (kernel, args, global_size, local_size) in statements
                ],
                "buffers": {
                    name: {"size": size, "dtype": dtype.name, "id": weight_names[_key] if _key in weight_names else ""}
                    for name, (size, dtype, _key) in bufs.items()
