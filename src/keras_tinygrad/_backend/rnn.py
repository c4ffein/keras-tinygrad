"""RNN kernels for the tinygrad backend.

The generic `rnn()` scan is a straight port of the numpy backend's
implementation, executed with tinygrad tensors end-to-end so gradients flow
...
def rnn(
    ...
    inputs = tree.map_structure(convert_to_tensor, inputs)

    def swap_batch_timestep(input_t):
        # Swap the batch and timestep dim for the incoming tensor.
        return input_t.transpose(0, 1)
    ...
    if mask is not None:
        ...
        if mask.dtype != tg_dtypes.bool:
            mask = mask.cast(tg_dtypes.bool)
        if len(mask.shape) == 2:
    ...
    def _expand_mask(mask_t, input_t, fixed_dim=1):
        ...
        return mask_t.repeat(multiples)
    ...
    if unroll:
        ...
        if mask is not None:
            ...
            for i in range(time_steps):
                ...
                if not successive_outputs:
                    prev_output = Tensor.zeros_like(output)
                else:
                    prev_output = successive_outputs[-1]

                output = tiled_mask_t.where(output, prev_output)
            ...
            outputs = Tensor.stack(*successive_outputs)
        ...
        if mask is not None:
            ...
            def _step(states, current_input):
                ...
                is_masked = current_mask.logical_not().all(
                    axis=-1, keepdim=True
                )
                ...
                if zero_output_for_mask:
                    masked_outs = is_masked.where(
                        Tensor.zeros_like(output_t), output_t
                    )
                else:
                    ...
                    masked_outs = is_masked.where(output_tm1, output_t)

                new_states = tree.map_structure(
                    lambda s, ns: is_masked.where(s, ns),
                    states,
                    new_states,
                )
        ...
        new_states, outputs = tinygrad_scan(
            f=_step,
            init=initial_states,
            xs=scan_xs,
...
def lstm(*args, **kwargs):
    raise NotImplementedError(
        "tinygrad backend: fused `lstm` is not implemented yet"
    )


def gru(*args, **kwargs):
    raise NotImplementedError(
        "tinygrad backend: fused `gru` is not implemented yet"
    )


def bidirectional_lstm(*args, **kwargs):
    raise NotImplementedError(
        "tinygrad backend: `bidirectional_lstm` is not implemented yet"
    )


def bidirectional_gru(*args, **kwargs):
    raise NotImplementedError(
        "tinygrad backend: `bidirectional_gru` is not implemented yet"
    )


def unstack(x, axis=0):
    return [
        x[(slice(None),) * axis + (i,)] for i in range(x.shape[axis])
    ]


def tinygrad_scan(f, init, xs, reverse=False, mask=None):
    states = init
    outputs = []

    if mask is not None:
        x, mask = xs
        x = x.flip(0) if reverse else x
        mask = mask.flip(0) if reverse else mask
